 ...
 */
public class SlidingTilesActivity extends ActionBarActivity implements Observer, ServiceConnection {
    /**
    ...
    protected void onStart() {
        super.onStart();
        bindService(new Intent(SlidingTilesActivity.this, UserManager.class), this, Context.BIND_AUTO_CREATE);
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (R.id.action_save == item.getItemId()) {
            if (userManager != null && userManager.getStatus()){
                ...
                Toast.makeText(SlidingTilesActivity.this, "Game Saved", Toast.LENGTH_SHORT).show();
            } else { Toast.makeText(SlidingTilesActivity.this, "You Must Login to Save", Toast.LENGTH_SHORT).show(); }
            return true;
        }
        else if (R.id.action_revert == item.getItemId()) {
            gesture.undo(SlidingTilesActivity.this);
            return true;
        }
        return false;
    }
