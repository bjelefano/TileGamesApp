...
import group_0580.gamecentre.gestures.GestureDetectGridView;
import group_0580.gamecentre.user.UserManager;
import group_0580.gamecentre.gestures.CustomAdapter;

/**
 * The game activity.
 */
public class GameActivity extends ActionBarActivity implements Observer, ServiceConnection {
    /**
    ...
    /**
     * True when the game wants to start the counter; false when the game wants to pause the counter
     */
    ...
    protected void onStart() {
        super.onStart();
        bindService(new Intent(GameActivity.this, UserManager.class), this, Context.BIND_AUTO_CREATE);
    }
    ...
    public boolean onOptionsItemSelected(MenuItem item) {
        if (R.id.action_save == item.getItemId()) {
            ...
            } else { Toast.makeText(GameActivity.this, "You Must Login to Save", Toast.LENGTH_SHORT).show(); }
            return true;
        }
        else if (R.id.action_revert == item.getItemId()) {
            gesture.undo(GameActivity.this);
            return true;
        }
        return false;
    }
    ...
    /**
     * Called when the game observer detects change.
     */
    @Override
    ...
    /**
     * Creates buttons according to the grid size and the number of tiles within the game. Assigns
    ...
    /**
     * Updates the Moves-Made TextView according to changes within the game.
     */
    ...
    private void initGame() {
        ...
        createTileButtons(GameActivity.this);
        initGrid();
    }

    /**
     * Checks if the game is over. If so, LeaderboardActivity is initiated.
     */
    private void gameOver() {
        if (game.isWon()) {
            Intent scoreboard = new Intent(GameActivity.this, LeaderBoardActivity.class);
            if (userManager != null && userManager.getStatus()) {
